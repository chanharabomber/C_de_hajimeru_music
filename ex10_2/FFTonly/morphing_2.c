...
int main(void)
{
  ...
  //mono_wave_read(&pcm00, "grouwlvoice.wav"); /* WAVEファイルからモノラルの音データを入力する */
...
/*
    ...
    for (k = 0; k < N; k++)
    {
      A[(int)(k*0.3)] = sqrt(x_real[k] * x_real[k] + x_imag[k] * x_imag[k]);
...
//        x2_real[n] = pcm0.s[n+frame];//
  ...
  //for(k=0;k<15*N;k++) {
      ...
      //printf("%d %f\n",k,pcm1.s[k]);
      //pcm1.s[k] = y_real[k];
    //pcm1.s[k+offset] += 2.0*y_real[k];//
    //pcm1.s[k+offset] += 0.1*y2_real[k];//
  }
